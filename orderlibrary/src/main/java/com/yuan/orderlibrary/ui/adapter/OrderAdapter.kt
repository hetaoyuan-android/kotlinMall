...
class OrderAdapter(context: Context) : BaseRecyclerViewAdapter<Order, OrderAdapter.ViewHolder>(context) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (model.orderGoodsList.size == 1){//单个商品
            holder.itemView.mSingleGoodsView.visibility= View.VISIBLE
            holder.itemView.mMultiGoodsView.visibility= View.GONE//单个商品隐藏多个商品视图
    ...
    private fun setOptVisible(confirmVisible: Boolean, waitPayVisible: Boolean, cancelVisible: Boolean,holder: ViewHolder) {
        if (confirmVisible) {
            holder.itemView.mConfirmBtn.visibility = View.VISIBLE
        } else {
            holder.itemView.mConfirmBtn.visibility = View.GONE
        }
        if (waitPayVisible) {
            holder.itemView.mPayBtn.visibility = View.VISIBLE
        } else {
            holder.itemView.mPayBtn.visibility = View.GONE
        }
        if (cancelVisible) {
            holder.itemView.mCancelBtn.visibility = View.VISIBLE
        } else {
            holder.itemView.mCancelBtn.visibility = View.GONE
        }

        if (confirmVisible or waitPayVisible or cancelVisible){
            holder.itemView.mBottomView.visibility = View.VISIBLE
        }else{
            holder.itemView.mBottomView.visibility = View.GONE
        }
